...
export async function getRecentLogs(userId, daysBack = 14) {
  await init();
  const cutoff = new Date(Date.now() - daysBack * 86400000).toISOString().slice(0, 10);
  const queryPromise = getClient().execute({
    sql: 'SELECT date, data FROM daily_logs WHERE user_id = ? AND date >= ? ORDER BY date',
    args: [userId, cutoff]
  });
  const timeoutPromise = new Promise((_, rej) => setTimeout(() => rej(new Error('DB query timeout after 8s')), 8000));
