...
export async function loadLogs(){const t=localStorage.getItem('token')||sessionStorage.getItem('token');const r=await fetch('/api/logs/load',{headers:{'Authorization':'Bearer '+t}});if(!r.ok)throw new Error('load logs failed');const d=await r.json();return d.logs||{};}
