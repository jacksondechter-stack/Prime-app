...
import { getUserByUsername, getRecentLogs } from '../../../../lib/db';
...
export async function POST(request) {
  try {
    ...
    const response = NextResponse.json({ success: true, token, user: { id: Number(user.id), username: user.username, name: user.name, profile }, logs, logsError });
    ...
    return response;
  } catch (e) { console.error('Login error:', e); return NextResponse.json({ error: 'Server error: ' + e.message }, { status: 500 }); }
}
