#!/usr/bin/env python3
"""
Surgery 6: Custom Exercise Full Integration

6b:   Removes 6-exercise cap on Today card; adds 3rd map param for borderBottom
6b-2: Dynamic borderBottom (no longer hardcoded i<5)
6c-1: Custom exercise name/border colors reflect done state
6c-2: Adds completion toggle button to custom exercises in Train tab
6d-1: wCal calc iterates ALL custom exercise keys (myEx, myExPush/Pull/Legs)
6e-1: Weekly average (wN) calc same fix
6e-2: NetStreak calc same fix
6f:   TT dots history calc adds custom exercise contribution
6g:   closeDay grade API sends all custom exercise keys
"""
import sys
from pathlib import Path

FILE = Path("src/components/PrimeApp.js")

def patch(content, find, replace, label):
    count = content.count(find)
    if count != 1:
        print(f"FAIL {label}: expected 1 match, found {count}")
        # Diagnostic: find first 60 chars of search and show what's actually there
        anchor = find[:60]
        if anchor in content:
            idx = content.find(anchor)
            actual = content[idx:idx+len(find)+30]
            print(f"  Anchor found at offset {idx}.")
            print(f"  ACTUAL (first {min(len(actual),200)} chars):")
            ...
            for i, (a, b) in enumerate(zip(actual, find)):
                if a != b:
                    print(f"  First difference at char {i}: actual={repr(a)} (U+{ord(a):04X}) vs expected={repr(b)} (U+{ord(b):04X})")
                    break
        else:
...
print("\nSurgery 6 complete. 9 patches applied.")
